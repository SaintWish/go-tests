...
// 	*shard[K, V] | *shardCapacity[K, V]
// }

type ShardMap[K comparable, V any] interface {
	has(key K) bool
	getHasRenew(key K) (V, bool)
	getHas(key K) (V, bool)

	getRenew(key K) V
	get(key K) V

	set(key K, val V, callback func(K, V))
	update(key K, val V)

	delete(key K) bool
	deleteCallback(key K, callback func(K, V)) bool

	clear()
	flush(callback func(K, V))
}
...
func New[K comparable, V any](sz uint64, sc uint64, resize bool) *Cache[K, V] {
	if sc > sz {
		panic("kvswiss2: shard count must be smaller than cache size!")
	}
